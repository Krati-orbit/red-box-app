...
const Cubes = ({
    ...
    const onTouchEnd = useCallback(() => { if (sceneRef.current) resetAll(); }, [resetAll]);
    ...
    useEffect(() => {
        ...
        const speed = 0.02;
        const loop = () => {
            if (!userActiveRef.current && sceneRef.current) {
    ...
    useEffect(() => {
        ...
        if (!global) el.addEventListener('pointerleave', resetAll);
        ...
        return () => {
            ...
            if (!global && el) el.removeEventListener('pointerleave', resetAll);
    ...
    }, [onPointerMove, resetAll, onClick, onTouchMove, onTouchStart, onTouchEnd, global]);
